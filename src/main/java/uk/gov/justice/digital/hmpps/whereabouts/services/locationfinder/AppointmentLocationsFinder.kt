...
class AppointmentLocationsFinder(
   ...
   */
  private val events = scheduledAppointments
    .flatMap {
      listOf(
   ...
   */
  private val locationOccupancy = locationIds.map { it to 0L }.toMap().toMutableMap()
   ...
   */
  private val locationsForAppointmentIntervals = appointmentIntervals.map { LocationsForAppointmentIntervals(it, null) }

  /**
   * Retrieve from the locationOccupancy map the set of locations that are currently unoccupied.
   */
  private fun unoccupiedLocations(): MutableSet<Long> = locationOccupancy.filterValues { it == 0L }.keys.toMutableSet()
   ...
   */
  fun findLocationsForAppointmentIntervals(): List<LocationsForAppointmentIntervals> {
