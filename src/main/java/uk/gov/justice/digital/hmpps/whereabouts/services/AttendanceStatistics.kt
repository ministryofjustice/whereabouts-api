...
data class UnpaidReasons(val refused: Int? = 0, val refusedWithIncentiveLevelWarning: Int?, val sessionCancelled: Int? = 0, val unacceptableAbsence: Int? = 0, val restInCell: Int? = 0, val restInCellOrSick: Int? = 0)
  ...
  fun getStats(prisonId: String, period: TimePeriod?, from: LocalDate, to: LocalDate): Stats {
    ...
    return Stats(
        ...
        unpaidReasons = UnpaidReasons(
            ...
            refusedWithIncentiveLevelWarning = attendances.count { it.absentReason == AbsentReason.RefusedWithIncentiveLevelWarning },
